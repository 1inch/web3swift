...
public class HDNode {
    ...
    public init?(_ data: Data) {
        ...
        childNumber = cNum.withUnsafeBytes { $0.load(as: UInt32.self) }
        chaincode = data[13..<45]
        if serializePrivate {
            privateKey = data[46..<78]
            guard let pubKey = Utils.privateToPublic(privateKey!, compressed: true) else {return nil}
        ...
        if derivePrivateKey {
            if self.hasPrivate { // derive private key when is itself extended private key
                ...
                if newNode.isHardened {
                    newPath = self.path! + "/"
                    newPath += "\(newNode.index % HDNode.hardenedIndexPrefix)" + "'"
                } else {
                    newPath = self.path! + "/" + "\(newNode.index)"
                }
