...
public class EthereumKeystoreV3: AbstractKeystore {
    ...
    public init?(_ keystoreParams: KeystoreParamsV3) {
        ...
        guard let addressString = keystoreParams.address,
    ...
    fileprivate func encryptDataToStorage(_ password: String, keyData: Data?, dkLen: Int=32, N: Int = 4096, R: Int = 6, P: Int = 1, aesMode: String = "aes-128-cbc") throws {
        ...
        let last16bytes = derivedKey.subdata(in: (derivedKey.count - 16)..<derivedKey.count)
        let encryptionKey = derivedKey.subdata(in: 0..<16)
...
//        let encryptedKeyData = Data(bytes:encryptedKey)
        ...
        guard let pubKey = Utils.privateToPublic(keyData!) else {throw AbstractKeystoreError.keyDerivationError}
        guard let addr = Utils.publicToAddress(pubKey) else {throw AbstractKeystoreError.keyDerivationError}
        self.address = addr
        let keystoreparams = KeystoreParamsV3(address: addr.hex(eip55: true), crypto: crypto, id: UUID().uuidString.lowercased(), version: 3)
        self.keystoreParams = keystoreparams
    }
    ...
    fileprivate func getKeyData(_ password: String) throws -> Data? {
        ...
        case "pbkdf2":
            ...
            switch algo {
                case "hmac-sha256" :
                    hashVariant = .sha2(.sha256)
                case "hmac-sha384" :
                    hashVariant = .sha2(.sha384)
                case "hmac-sha512" :
                    hashVariant = .sha2(.sha512)
                default:
                    hashVariant = nil
            }
...
//            passwordDerivedKey = Data(bytes:derivedArray)
        ...
        var dataForMAC = Data()
        let derivedKeyLast16bytes = derivedKey.subdata(in: (derivedKey.count - 16)..<derivedKey.count)
