...
public class BIP39 {
    ...
    */
    ...
    static public func isWordsCountValid(_ count: Int) -> Bool {
        count >= 12 && count.isMultiple(of: 3)
    }
    
    static public func mnemonicsToEntropy(_ wordList: [String], language: BIP39Language = BIP39Language.english) -> Data? {
        guard isWordsCountValid(wordList.count) else {
            return nil
        }
        var bitString = ""
    ...
    static public func seedFromMmemonics(_ mnemonics: String, password: String = "", language: BIP39Language = BIP39Language.english) -> Data? {
        ...
        guard let seedArray = try? PKCS5.PBKDF2(password: mnemData.bytes, salt: saltData.bytes, iterations: 2048, keyLength: 64, variant: .sha2(.sha512)).calculate() else {return nil}
